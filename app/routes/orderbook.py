...
def get_latest_order_book():
    # Get the most recent order book snapshot using the highest timestamp score
...
# Fetch order books within a specific range of timestamps
@router.get("/range")
def get_order_book_in_range(start_ts: int, end_ts: int):
    # Fetch order books within a given timestamp range
...
# Paginate the order books
@router.get("/paginate")
def paginate_order_book(start: int = 0, end: int = 9):
    # Fetch a range of order books for pagination
