...
env_path = current_dir.parent / '.env'  # Look in parent directory (trading-backend root)
...
class Settings:
    ...
    GOOGLE_REDIRECT_URI = os.getenv("GOOGLE_REDIRECT_URI", "http://localhost:8000/authorize")
    ...
    def validate(cls):
        ...
        if missing:
            logger.warning(f"Missing environment variables: {', '.join(missing)} - Some features may not work")
            logger.warning("For testing purposes, you can continue without these variables")
            return
